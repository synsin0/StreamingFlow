...
class DefaultFormatBundle3D:
    ...
    def __call__(self, results):
        ...
        if 'raydirs' in results:
            if isinstance(results['raydirs'], list):
                raydirs = [raydir for raydir in results['raydirs']]
                raydirs = np.ascontiguousarray(np.stack(raydirs, axis=0))
                results['raydirs'] = DC(to_tensor(raydirs), stack=True)

            if isinstance(results['lightpos'], list):
                lightposes = [lightpos for lightpos in results['lightpos']]
                lightposes = np.ascontiguousarray(np.stack(lightposes, axis=0))
                lightposes = to_tensor(lightposes)
                lightposes = lightposes.unsqueeze(1).repeat(1, raydirs.shape[1], 1)
                results['lightpos'] = DC(lightposes, stack=True)

            if isinstance(results['gt_images'], list):
                gt_images = [gt_image for gt_image in results['gt_images']]
                gt_images = np.ascontiguousarray(np.stack(gt_images, axis=0))
                results['gt_images'] = DC(to_tensor(gt_images), stack=True)

            if isinstance(results['gt_depths'], list) and len(results['gt_depths'])!=0:
                gt_depths = [gt_depth for gt_depth in results['gt_depths']]
                gt_depths = np.ascontiguousarray(np.stack(gt_depths, axis=0))
                results['gt_depths'] = DC(to_tensor(gt_depths), stack=True)

            if isinstance(results['denorm_images'], list):
                # process multiple imgs in single frame
                imgs = [img.transpose(2, 0, 1) for img in results['denorm_images']]
                imgs = np.ascontiguousarray(np.stack(imgs, axis=0))
                results['denorm_images'] = DC(to_tensor(imgs).float(), stack=True)
