    ...
    def __call__(self, input_dict):
        ...
        if input_dict['pcd_horizontal_flip']:
            ...
            if "points" in input_dict:
                for frame in range(len(input_dict['points'])):
                    input_dict["points"][frame].flip("horizontal")
        ...
        if input_dict['pcd_vertical_flip']:
            ...
            if "points" in input_dict:
                for frame in range(len(input_dict['points'])):
                    input_dict["points"][frame].flip("vertical")
...
class MTLGlobalRotScaleTrans(object):
    ...
    def _trans_bbox_points(self, input_dict):
        ...
        if 'points' in input_dict:
            for frame in range(len(input_dict['points'])):
                input_dict['points'][frame].translate(trans_factor)
    ...
    def robust_rot_bbox_points(self, input_dict):
        ...
        for key in input_dict['bbox3d_fields']:
            ...
            for frame in range(len(input_dict[key])):
                if input_dict[key][frame] is not None and len(input_dict[key][frame].tensor) > 0:
                    if 'points' in input_dict and frame < len(input_dict['points']):
                        points, rot_mat_T = input_dict[key][frame].rotate(
                        noise_rotation, input_dict['points'][frame])
                        input_dict['points'][frame] = points
                        input_dict['pcd_rotation'] = rot_mat_T
                    else:
    ...
    def _scale_bbox_points(self, input_dict):
        ...
        if 'points' in input_dict:
            for frame in range(len(input_dict['points'])):
                points = input_dict['points'][frame]
                points.scale(scale)
