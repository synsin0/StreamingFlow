...
@PIPELINES.register_module()
class MultiViewPipelineBEVDetForNeRF:
    def __init__(self,
        n_images,
        mean=[123.675, 116.28, 103.53],
        std=[58.395, 57.12, 57.375],
    ...
    def __call__(self, results):
        ...
        # if self.loading == 'random':
        #     ids = np.arange(results['img_inputs'][0].shape[0])
        #     replace = True if self.n_images > len(ids) else False
        #     ids = np.random.choice(ids, self.n_images, replace=replace)
        #     if self.nerf_target_views != 0:
        #         target_id = np.random.choice(
        ...
        ids = np.arange(len(results['img'])) 
        target_id = ids

        # imgs, sensor2egos, ego2globals, intrinsics_nerf, post_rots, post_trans, bda = results['img_inputs']
        ...
        for img in results['img']:
            denorm_img = mmcv.imdenormalize(
                    img.permute(1,2,0).cpu().numpy(), self.mean, self.std, to_bgr=False
                ).astype(np.uint8) / 255.0
            denorm_imgs_list.append(denorm_img)
        ...
        #     n_past = len(results['adjacent'])
        # else:
        #     n_past = 0
        # if 'future' in results:
        #     n_future = len(results['future'])
        # else:
        #     n_future = 0

        # if "ray_info" in results.keys():
        # results["c2w"], results["camrotc2w"], results["lightpos"] = results['ray_info']['c2w'], results['ray_info']['camrotc2w'], results['ray_info']['lightpos']
        ...
        assert self.nerf_target_views > 0
        for i in target_id:
            c2ws.append(results["camera2lidar"][i])
            camrotc2ws.append(results["camera2lidar"][i][:3,:3])
            lightposes.append(results["camera2lidar"][i][:3,3])
            px, py = np.meshgrid(
                np.arange(self.margin, width - self.margin).astype(np.float32),
                np.arange(self.margin, height- self.margin).astype(np.float32)
            )
            ...
            pixels.append(pixelcoords)
            raydir = get_dtu_raydir(
                pixelcoords, results['camera_intrinsics'][i], results["camera2lidar"][i][:3,:3])
            raydirs.append(np.reshape(raydir.astype(np.float32), (-1, 3)))
            
            # denormalize target_images.
            denorm_imgs = mmcv.imdenormalize(
                results['img'][i].permute(1,2,0).cpu().numpy(), self.mean, self.std, to_bgr=False
            )
            gt_rgb_shape = denorm_imgs.shape
        ...
        results['img_shape'] = list(img_shape)
        results['lidar2img'] = dict()
        
        post_transforms = np.stack(results['img_aug_matrix'])


        results['lidar2img']['intrinsic'] = np.stack(results['camera_intrinsics'])
        results['lidar2img']['extrinsic'] = np.stack(results['lidar2camera'])
        results['lidar2img']['post_transform'] = np.stack(results['img_aug_matrix'])
        results['lidar2img']['n_cameras'] = len(results['img'])
        return results
