...
from luban.experiment.trace import Tracer

########################################################################################################################
# helper functions for nerf ray rendering
########################################################################################################################
@Tracer(op_name="volume_sampling")
def volume_sampling(sample_pts, features, aabb, bda=None):
    ...
    inverse_bda_sample_pts = sample_pts @ bda    

    sample_pts = sample_pts.view(1, N_rays*N_samples, 1, 1, 3).repeat(B, 1, 1, 1, 1)
    inverse_bda_sample_pts = inverse_bda_sample_pts.view(1, N_rays*N_samples, 1, 1, 3).repeat(B, 1, 1, 1, 1)

    aabbSize = aabb[1] - aabb[0]
    invgridSize = 1.0/aabbSize * 2
    

    norm_pts = (inverse_bda_sample_pts-aabb[0]) * invgridSize - 1
...
@Tracer(op_name="_compute_projection")
def _compute_projection(img_meta, device = None, curr_frame = None):
    # [n_views, 34], 34 = img_size(2) + intrinsics(16) + extrinsics(16)
    

    projection = []
    views = len(img_meta['lidar2img']['extrinsic'])
    n_cameras = img_meta['lidar2img']['n_cameras']

    # intrinsic = torch.eye(4).unsqueeze(0).repeat(views,1,1)
    intrinsic = torch.Tensor(img_meta['lidar2img']['intrinsic']).to(device)
    # print(img_meta['lidar2img']['intrinsic'][:4, :4], img_meta['ori_shape'], img_meta['img_shape'])
    intrinsic = intrinsic.view(views, 16)
    

    img_size = torch.Tensor(img_meta['img_shape'][1:]).to(intrinsic.device)
    img_size = img_size.unsqueeze(0).repeat(views, 1)
    # use predicted pitch and roll for SUNRGBDTotal test


    extrinsic = torch.Tensor(img_meta['lidar2img']['extrinsic']).to(device).contiguous().view(views, 16)
    post_trans = torch.Tensor(img_meta['lidar2img']['post_transform']).to(device).contiguous().view(views, 16)
    train_cameras = torch.cat([img_size, intrinsic, extrinsic, post_trans], dim=-1)
    

    if curr_frame is not None:
        train_cameras = train_cameras[n_cameras * curr_frame: n_cameras * (curr_frame + 1)]
...
@Tracer(op_name="sample_along_camera_ray")
...
@Tracer(op_name="render_rays_func")
def render_rays_func(ray_o,
    ...
    if mode == "image":
    ...
    elif mode == "occ_volume":
        globalpts, inbound_masks = volume_sampling(pts, mean_volume, aabb, bda)
        # cov_pts, inbound_masks = volume_sampling(pts, cov_volume, aabb)
        # This masks is for indicating which points outside of aabb
        ...
        train_camera = _compute_projection(img_meta, img.device, curr_frame).to(img.device)

        _, view_mask = projector.compute(pts, img, train_camera, None)
        pixel_mask = view_mask[..., 0].sum(dim=2) > 0
        # plot_3D_vis(pts, aabb, img, train_camera)
        # [N_rays, N_samples], should at least have 2 observations
        # This mask is for indicating which points do not have projected point
        # globalpts = torch.cat([mean_pts, cov_pts], dim=-1)
