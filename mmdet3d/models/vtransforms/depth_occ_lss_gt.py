...
from mmdet3d.ops.occ_pooling import occ_pool



__all__ = ["DepthOccLSSTransformGT"]


@VTRANSFORMS.register_module()
class DepthOccLSSTransformGT(BaseDepthTransform):
    def __init__(
        self,
        in_channels: int,
        out_channels: int,
    ...
    @force_fp32()
    def occ_pool(self, geom_feats, x):
        B, N, D, H, W, C = x.shape
        Nprime = B * N * D * H * W
        ...
        final = final.permute(0, 1, 3, 4, 2)  # XYZ   # torch.Size([2, 80, 360, 360, 1])
        # collapse Z
