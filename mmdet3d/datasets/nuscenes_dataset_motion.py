...
class NuScenesMotionDataset(Custom3DDataset):
    r"""NuScenes Dataset.
    ...
    def prepare_train_data(self, index):
        ...
        """
        queue = []
        index_list = list(range(index-self.receptive_field, index))
    ...
    def union2one(self, queue):
        ...
        camera_intrinsics_list = [each['camera_intrinsics'].data for each in queue]
        camera2ego_list = [each['camera2ego'].data for each in queue]
        lidar2ego_list = [each['lidar2ego'].data for each in queue]
        lidar2camera_list = [each['lidar2camera'].data for each in queue]
        camera2lidar_list = [each['camera2lidar'].data for each in queue]
        lidar2image_list = [each['lidar2image'].data for each in queue]
        img_aug_matrix_list = [each['img_aug_matrix'].data for each in queue]
        lidar_aug_matrix_list = [each['lidar_aug_matrix'].data for each in queue]
        ...
        queue[-1]['camera_intrinsics'] = DC(torch.stack(camera_intrinsics_list), cpu_only=False, stack=True)
        queue[-1]['camera2ego'] = DC(torch.stack(camera2ego_list), cpu_only=False, stack=True)
        queue[-1]['lidar2ego'] = DC(torch.stack(lidar2ego_list), cpu_only=False, stack=True)
        queue[-1]['lidar2camera'] = DC(torch.stack(lidar2camera_list), cpu_only=False, stack=True)
        queue[-1]['camera2lidar'] = DC(torch.stack(camera2lidar_list), cpu_only=False, stack=True)
        queue[-1]['lidar2image'] = DC(torch.stack(lidar2image_list), cpu_only=False, stack=True)
        queue[-1]['img_aug_matrix'] = DC(torch.stack(img_aug_matrix_list), cpu_only=False, stack=True)
        queue[-1]['lidar_aug_matrix'] = DC(torch.stack(lidar_aug_matrix_list), cpu_only=False, stack=True)
    ...
    def get_egomotions(self, indices, lidar2ego):
        ...
        for index in indices:
            ...
            if index != -1 and next_frame < len(self.data_infos) and self.data_infos[next_frame]['scene_token'] == cur_info['scene_token']:
                ...
                # cur_egopose =  get_lidar2global(cur_info) 
                # next_egopose =  get_lidar2global(next_info)
    ...
    def get_data_info(self, index: int) -> Dict[str, Any]:
        ...
        # [num_seq, 6 DoF]
        future_egomotions = self.get_egomotions(all_frames, lidar2ego)
            ...
            for i, box in enumerate(boxes):
                ...
                if np.sqrt(box.velocity[0] ** 2 + box.velocity[1] ** 2) > 0.2:
                    if name in [
                        ...
                        attr = NuScenesMotionDataset.DefaultAttribute[name]
                else:
