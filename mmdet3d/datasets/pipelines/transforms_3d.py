...
class ImageAug3D:
    ...
    def __call__(self, data: Dict[str, Any], seed = None) -> Dict[str, Any]:
        if seed is not None:
            ...
            if isinstance(gt_boxes, list):
                for gtbox in gt_boxes:
                    if gtbox is not None:
                        rotation = np.eye(3)
                        rotation = rotation @ gtbox.rotate(theta).numpy()
                        gtbox.translate(translation)
                        gtbox.scale(scale)   
            else:
                rotation = np.eye(3)
    ...
    def __call__(self, data: Dict[str, Any], seed=None) -> Dict[str, Any]:
        ...
        if flip_horizontal:
            ...
            if "gt_bboxes_3d" in data:
                if isinstance(data["gt_bboxes_3d"], list):
                    for gtbox in data["gt_bboxes_3d"]:
                        if gtbox is not None:
                            gtbox.flip("horizontal")
                else:
