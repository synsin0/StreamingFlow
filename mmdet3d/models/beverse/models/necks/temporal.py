...
from ..dense_heads.map_head import BevFeatureSlicer
...
class Temporal3DConvModel(BaseModule):
    def __init__(
        ...
        # self.bev_slicer = BevFeatureSlicer(det_grid_conf, grid_conf)
        # self.spatial_extent = (grid_conf['xbound'][1], grid_conf['ybound'][1])
        # self.receptive_field = receptive_field
    ...
    def forward(self, x, future_egomotion=None, aug_transform=None, img_is_valid=None):
        B, T = x.shape[:2]
        x = self.bev_slicer(x.flatten(0,1))
        C, H, W = x.shape[1:]
        x = x.view(B,T, C, H, W)        
