...
from streamingflow.utils.geometry import mat2pose_vec, pose_vec2mat, warp_features
...
def make_instance_id_temporally_consistent_short_interval(pred_inst, future_flow=None, matching_threshold=10.0):
    """
    Parameters
    ----------
        pred_inst: torch.Tensor (1, seq_len, h, w)
...
def predict_instance_segmentation_and_trajectories_short_interval(
        output, compute_matched_centers=False, make_consistent=True, vehicles_id=1,
):
    preds = output['segmentation'].detach()
    preds = torch.argmax(preds, dim=2, keepdim=True)
    ...
    if make_consistent:
        ...
        for b in range(batch_size):
            consistent_instance_seg.append(
                make_instance_id_temporally_consistent_short_interval(pred_inst[b:b+1],
                                                       output['instance_flow'][b:b+1].detach())
            )
        consistent_instance_seg = torch.cat(consistent_instance_seg, dim=0)
    else:
