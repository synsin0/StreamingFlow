...
try:
    ...
    print('install petrel_client for using ceph')
...
class LoadMultiViewImageFromFiles:
    ...
    def __init__(self, to_float32=False, color_type="unchanged", use_ceph_data=False):
        self.to_float32 = to_float32
        self.color_type = color_type
        self.use_ceph_data = use_ceph_data
        if use_ceph_data:
            self.mclient = CephClient()
    
    def read_img(self, fname):
        if self.use_ceph_data:
            fname = fname.replace('./data/nuscenes/', 'nuscenes:s3://openmmlab/datasets/detection3d/nuscenes/')
            fname = fname.replace('data/nuscenes/', 'nuscenes:s3://openmmlab/datasets/detection3d/nuscenes/')
            value = self.mclient.Get(fname)
...
class LoadPointsFromMultiSweeps:
    ...
    def _load_points(self, lidar_path):
        ...
        """
        assert not self.load_augmented, f"wrong argment load_augmented: {self.load_augmented}"
        if self.use_ceph_data:
            lidar_path = lidar_path.replace('./data/nuscenes/', 'nuscenes:s3://openmmlab/datasets/detection3d/nuscenes/')
            lidar_path = lidar_path.replace('data/nuscenes/', 'nuscenes:s3://openmmlab/datasets/detection3d/nuscenes/')
            pts_bytes = self.mclient.get(lidar_path)
