...
class MTLEgoNuScenesDataset(NuScenesDataset):
    ...
    def get_data_info(self, index):
        ...
        past_and_present_frames = prev_indices + [index]

        input_dict = dict(
            sample_idx=info['token'],
            pts_filename=[self.data_infos[frame]['lidar_path'] for frame in past_and_present_frames],
            sweeps=[self.data_infos[frame]['sweeps'] for frame in past_and_present_frames],
            timestamp=[self.data_infos[frame]['timestamp'] / 1e6 for frame in past_and_present_frames],
