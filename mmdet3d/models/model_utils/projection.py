...
class Projector():
    ...
    def compute_projections(self, xyz, train_cameras):
        ...
        train_poses = train_cameras[:, 18:34].reshape(-1, 4, 4)  # [n_views, 4, 4]
        train_post_trans = train_cameras[:, 34:50].reshape(-1, 4, 4)  # [n_views, 4, 4]
        ...
        projections = projections.permute(0,2,1).bmm(train_post_trans).permute(0,2,1)  
    ...
    #     query_pose = query_camera[18:34].reshape(-1, 4, 4).repeat(num_views, 1, 1)  # [n_views, 4, 4]
    ...
    def compute(self, xyz, train_imgs, train_cameras, featmaps=None, grid_sample=True):
        '''
