...
from streamingflow.utils.instance import predict_instance_segmentation_and_trajectories_short_interval
...
def mk_save_dir():
    ...
    save_path = pathlib.Path('visualize_imgs_streaming') / string
    save_path.mkdir(parents=True, exist_ok=False)
    return save_path

def eval(checkpoint_path, continuous=False, dataroot=None,  n_future_frames=4, eval_interval = 10, draw=False):
    if draw:
    ...
    for index, batch in enumerate(tqdm(valloader)):
        ...
        #     print('Indice {} is not valid!'.format(index))
        #     continue
        ...
        if eval_interval != 1:
            B, T = target_timestamp.shape
            T_new = model.receptive_field  + int((T - model.receptive_field) / eval_interval)
            target_timestamp_new = torch.zeros([B, T_new]).to(target_timestamp.device).type(target_timestamp.dtype)
            target_timestamp_new[:,:model.receptive_field -1] = target_timestamp[:,:model.receptive_field -1]
            target_timestamp_new[:,model.receptive_field - 1:] = target_timestamp[:,model.receptive_field - 1:][:,::eval_interval]
            target_timestamp = target_timestamp_new
            batch['target_timestamp'] = target_timestamp
        ...
        metric_vehicle_val(seg_prediction[:, n_present - 1:], labels['segmentation'][:, n_present - 1:][:,::eval_interval])
        ...
        if cfg.INSTANCE_SEG.ENABLED:
            pred_consistent_instance_seg = predict_instance_segmentation_and_trajectories_short_interval(
                output, compute_matched_centers=False, make_consistent=True
            )
            metric_panoptic_val(pred_consistent_instance_seg[:, n_present - 1:],
                                     labels['instance'][:, n_present - 1:][:,::eval_interval])
        ...
        if draw and index % draw_interval ==0:
            ...
            #         output, compute_matched_centers=False, make_consistent=True
            #     )
            pred_consistent_instance_seg = predict_instance_segmentation_and_trajectories_short_interval(   # do not use instance flow at post-processing
                    output, compute_matched_centers=False, make_consistent=True
                )
...
def plot_prediction(batch, labels, image, output, consistent_instance_seg,index_t,frame,save_path,cfg):
    ...
    # Process predictions
    consistent_instance_seg = predict_instance_segmentation_and_trajectories_short_interval(
...
if __name__ == '__main__':
    ...
    parser.add_argument('--future-frames', default=True, type=int)
    parser.add_argument('--eval-interval', default=1, type=int) # 0.1s

    args = parser.parse_args()

    eval(args.checkpoint, args.continuous, args.dataroot, args.future_frames, args.eval_interval)
